  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      scope:
        ...
        initialData.applicableCategories.length > 0
          ? 'CATEGORIES'
          : 'ALL',
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const submissionValues = {
      ...
      applicableCategories:
        values.scope === 'ALL' ? [] : values.applicableCategories,
    };
    ...
    try {
      ...
      toast.success(
        initialData ? 'แก้ไขโปรโมชั่นสำเร็จ' : 'เพิ่มโปรโมชั่นสำเร็จ',
      );
      onSuccess();
    } catch (err: any) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                ชื่อโปรโมชั่น <span className="text-destructive">*</span>
              </FormLabel>
              <FormControl>
                <Input
                  placeholder="เช่น ส่วนลดไตเติ้ลรักพรีม 67 บาท"
                  {...field}
                />
              </FormControl>
              <FormMessage />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                ประเภทโปรโมชั่น <span className="text-destructive">*</span>
              </FormLabel>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                ขอบเขตการใช้งาน <span className="text-destructive">*</span>
              </FormLabel>
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl>
                  <SelectTrigger>
                ...
                <SelectContent>
                  <SelectItem value="ALL">
                    ใช้ได้กับสินค้าทุกรายการ (ทั้งร้าน)
                  </SelectItem>
                  <SelectItem value="CATEGORIES">
                    เฉพาะหมวดหมู่สินค้าที่เลือก
                  </SelectItem>
                </SelectContent>
              </Select>
              <FormMessage />
        ...
        {watchScope === 'CATEGORIES' && (
          <div className="space-y-2 bg-muted/30 p-3 rounded-lg border border-dashed animate-fade-in-scale">
            <FormLabel className="text-xs font-semibold">
              เลือกหมวดหมู่ที่เข้าร่วมโปรโมชั่น
            </FormLabel>
            {categories.length === 0 ? (
              <p className="text-xs text-muted-foreground py-2">
                ไม่พบหมวดหมู่สินค้าในระบบ (กรุณาเพิ่มสินค้าที่มีหมวดหมู่ก่อน)
              </p>
            ) : (
              <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto pt-1">
                {categories.map((cat) => {
                  ...
                  return (
                    <label
                      key={cat}
                      className="flex items-center gap-2 text-sm cursor-pointer select-none hover:bg-muted/50 p-1.5 rounded transition-colors"
                    >
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={() => {
                          if (isChecked) {
                            form.setValue(
                              'applicableCategories',
                              watchApplicableCategories.filter(
                                (c) => c !== cat,
                              ),
                            );
                          } else {
                            form.setValue('applicableCategories', [
                              ...watchApplicableCategories,
                              cat,
                            ]);
                          }
                        }}
        ...
        {watchType === 'FIXED_DISCOUNT' && (
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  ส่วนลด (บาท) <span className="text-destructive">*</span>
                </FormLabel>
                <FormControl>
        ...
        {watchType === 'PERCENT_DISCOUNT' && (
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  ส่วนลด (%) <span className="text-destructive">*</span>
                </FormLabel>
                <FormControl>
                  <Input
                    type="number"
          ...
          <div className="grid grid-cols-2 gap-4 bg-muted/30 p-3 rounded-lg border border-dashed">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    ซื้อครบ (ชิ้น) <span className="text-destructive">*</span>
                  </FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    ราคาพิเศษ (บาท) <span className="text-destructive">*</span>
                  </FormLabel>
                  <FormControl>
                    <Input type="number" min="0" step="0.01" {...field} />
                  </FormControl>
